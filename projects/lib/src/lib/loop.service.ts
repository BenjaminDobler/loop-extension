...
export class LoopService {
  ...
  addMarkerAtCurrentTime() {
    ...
    const nextMarker =
      newMarkers.length > newMarkerIndex + 1
        ? newMarkers[newMarkerIndex + 1]
        : null;
    const previousMarker =
      newMarkerIndex - 1 >= 0 ? newMarkers[newMarkerIndex - 1] : null;
    ...
    if (!nextMarker && previousMarker) {
      ...
      const existingLoop = newLoops.find(
        (l) => l.start.id === previousMarker.id && l.end.id === nextMarker.id
      );
      if (existingLoop) {
  ...
  removeMarker(marker: Marker) {
    ...
    const nextMarker =
      markers.length > markerIndex + 1 ? markers[markerIndex + 1] : null;
    const previousMarker =
      markerIndex - 1 >= 0 ? markers[markerIndex - 1] : null;
  ...
  updateMarkerTime(marker: Marker, newTime: number) {
    marker.time = newTime;
    marker.percentage = (newTime / this._duration) * 100;
    let markers = this.markers.getValue();
  ...
  updateVideo() {
    ...
    video?.addEventListener('timeupdate', (e) => {
      ...
      this.videoData.next({
        ...this.videoData.getValue(),
        currentPercentage: this._currentPercentage,
